# 研修について
## 概要・目的
- 業務経験が重要視されるので、「業務と同等の経験を積む」ことを目的とし、実際の業務と同じように運用します。
- githubにてタスク管理を行い、都度技術的な内容の調査・学習をしてもらいます。
- 雑な指示をする場合もあります。失敗、ミスを誘導することもあります。失敗、ミスからのリカバリが重要です。
...
- githubアカウント作成
  - 会社のメールアドレスでアカウント作成
      https://github.co.jp/
- 作業フォルダ、ツール保存先
必須ではなく、推奨。ただし自分で把握できる場所にすること。
  - 作業フォルダ：C:\Users\[各ユーザ名]\Documents\work
  - ツール保存先：C:\Users\[各ユーザ名]\Documents\tools
- スキルイメージ説明
- Markdown環境作成・学習
- github概要説明
- github基本コマンド説明
