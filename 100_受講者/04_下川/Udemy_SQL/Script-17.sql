...
SELECT * FROM students WHERE height<ALL(SELECT height+10 FROM students WHERE class_no IN(3,4))AND class_no = 1;
...
SELECT *,
CASE 
  WHEN COALESCE(tests_score_1,tests_score_2,tests_score_3)>=900
  THEN FLOOR(COALESCE(tests_score_1,tests_score_2,tests_score_3)*1.2) 
  WHEN COALESCE(tests_score_1,tests_score_2,tests_score_3)<=600
  THEN CEILING(COALESCE(tests_score_1,tests_score_2,tests_score_3)*0.8)
  ELSE COALESCE(tests_score_1,tests_score_2,tests_score_3)
END AS results
FROM tests_score;
